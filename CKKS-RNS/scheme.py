...
class Scheme:
    ...
    def gen_sk(self, primes, debug=0):
        M = prod(primes)
    ...
    def he_mul(self, ax1, bx1, ax2, bx2, relin_key_ax_qipi, relin_key_bx_qipi, qi,pi, sk_qi_dbg,m1_qi_dbg, m2_qi_dbg):
        P_dbg = prod(pi)
        Q_dbg = prod(qi)
        QP_dbg = Q_dbg*P_dbg
        qipi=qi.copy()
        qipi.extend(pi)
        print('qipi: ',qipi)
        d0_qi = self.polyEval.mul(bx1, bx2, qi)
        ...
        d1_qi = add(self.polyEval.mul(bx1, ax2, qi), self.polyEval.mul(bx2, ax1, qi), qi)
        print('d1: ', crt_to_coef(d1_qi,qi))
        tmp_dbg = ax1_dbg*bx2_dbg + ax2_dbg*bx1_dbg
        print('a1b2+a2b1: ', self.modulo(tmp_dbg, Q_dbg))
        print('-----')

        d2_qi = self.polyEval.mul(ax1, ax2, qi)
        print('d2_qi: ', d2_qi)
        print('d2_qi_mul: ', self.polyEval.mul(ax1, ax2, qi))
        ...
        print('A*sk mod qi: ', coef_to_crt(self.modulo(A * sk_dbg, Q_dbg), qi))
        print('-----')
        print('mul(d2_qipi,relin_key_bx_qipi,qipi): ', self.polyEval.mul(d2_qipi, relin_key_bx_qipi, qipi))
        B2 = add(d0_qi, mod_down(self.polyEval.mul(d2_qipi, relin_key_bx_qipi, qipi), qipi, qi), qi)

        print('B2: ', crt_to_coef(B2, qi))
        print('B2_qi: ', B2)
        # print('B2_qi_tmp: ', B2_tmp)

        A2 = add(add(self.polyEval.mul(ax1, bx2, qi), self.polyEval.mul(ax2, bx1, qi), qi), mod_down(self.polyEval.mul(d2_qipi, relin_key_ax_qipi, qipi), qipi, qi), qi)
        print('A2=',crt_to_coef(A2,qi))
        print('B2_qi-A2_qi*sk_qi: ', crt_to_coef(sub(B2, self.polyEval.mul(A2, sk_qi_dbg, qi), qi), qi))
        print('A2_qi*sk_qi: ', self.polyEval.mul(A2, sk_qi_dbg, qi))
        print('A2*sk coef : ', crt_to_coef(self.polyEval.mul(A2, sk_qi_dbg, qi), qi))
    ...
    def he_add(self, ax1, bx1, ax2, bx2, qi):
        return add(ax1,ax2,qi), add(bx1,bx2,qi)


    def encrypt(self, pt_qi, sk_qi, qi, debug=0):
        ...
        ax_time_sk_qi = self.polyEval.mul(ax_qi, sk_qi, qi)
        bx_qi = add(ax_time_sk_qi, pt_qi, qi)
        bx_qi = add(bx_qi, noise,qi)
        return ax_qi, bx_qi


    def decrypt(self, ax_qi, bx_qi, sk_qi, qi):
        ax_time_sk_qi = self.polyEval.mul(ax_qi, sk_qi, qi)
        return sub(bx_qi,ax_time_sk_qi, qi)


    def gen_relin_key(self, sk_qipi, qipi, pi):
        P = prod(pi)
        ax_qipi = gen_rand_poly_crt(qipi)
        ax_time_sk_qipi = self.polyEval.mul(ax_qipi, sk_qipi, qipi)
        sk_sqr_qipi = self.polyEval.mul(sk_qipi, sk_qipi, qipi)
